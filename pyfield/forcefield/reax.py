"""ReaxFF parser/writer.

Logic ported verbatim from the pre-Phase-1 top-level `ForceField.py` — see
the `test_parser_roundtrip` regression that pins this behaviour. Cleanups
(state machine, dataclasses) are explicitly deferred to Phase 2.
"""
import re

from pyfield.forcefield import constants as REAXConstants
from pyfield.forcefield.base import ForceField, list_to_dict


class REAX_FF(ForceField):
    """ReaxFF force-field parsed from a `ffield.reax` file.

    Attributes
    ----------
    param_selection : list[tuple]
        `(section, entry, item)` tuples the optimizer is allowed to move.
    params : dict
        Nested dict `params[section][entry][item] -> float`. For the GENERAL
        section the second level is the entry and the third is always 1.
    """
    ...
    def __init__(self, ff_filePath, ParamSelect_filePath):
        ...
        for line_item in self.reaxFile:
            ...
            if GENERAL_flag:
                self.Num_Of_GENERAL = int("".join(re.findall(r"\d", line_item[0:GENERAL_flag.span()[0]])))
            ...
            if ATOMS_flag:
                self.Num_Of_Atoms = int("".join(re.findall(r"\d", line_item[0:ATOMS_flag.span()[0]])))
                for j in range(self.Num_Of_Atoms):
                    ...
                    for k in range(4):
                        ...
                        if re.findall("[A-Z]" + "[a-z]", self.reaxFile[templine_index]):
                            ...
                            self.removed_parts_of_FField.append("      ")
            ...
            if BONDS_flag:
                self.Num_Of_BONDS = int("".join(re.findall(r"\d", line_item[0:BONDS_flag.span()[0]])))
            ...
            if OFF_DIAG_flag:
                self.Num_Of_OFF_DIAG = int("".join(re.findall(r"\d", line_item[0:OFF_DIAG_flag.span()[0]])))
            ...
            if ANGLES_flag:
                self.Num_Of_ANGLES = int("".join(re.findall(r"\d", line_item[0:ANGLES_flag.span()[0]])))
            ...
            if TORSIONS_flag:
                self.Num_Of_TORSIONS = int("".join(re.findall(r"\d", line_item[0:TORSIONS_flag.span()[0]])))
            ...
            if H_BONDS_flag:
                self.Num_Of_H_BONDS = int("".join(re.findall(r"\d", line_item[0:H_BONDS_flag.span()[0]])))
    ...
    def parseParamSelectionFile(self):
        ...
        for j in range(self._param_selected):
            while '' in self.Param_read[j]:
                self.Param_read[j].remove('')
        for i in range(self._param_selected):
            ...
            self.param_min_max_delta[temp_tuple] = {
                'delta': float(self.Param_read[i][3]),
                'min': float(self.Param_read[i][4]),
                'max': float(self.Param_read[i][5]),
            }
        temp_file.close()
