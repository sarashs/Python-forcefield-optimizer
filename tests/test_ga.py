...
def test_random_within_bounds_respects_min_max(ff):
    rng = random.Random(0)
    snap = _random_within_bounds(ff, rng)
...
def test_crossover_swaps_at_a_single_point(ff):
    rng = random.Random(1)
    a = _random_within_bounds(ff, rng)
    b = _random_within_bounds(ff, rng)
    # Force crossover by setting rate=1.0
    c1, c2 = _crossover(a, b, rate=1.0, rng=rng)
...
def test_crossover_clones_when_rate_zero(ff):
    ...
    a = _random_within_bounds(ff, rng)
    b = _random_within_bounds(ff, rng)
    c1, c2 = _crossover(a, b, rate=0.0, rng=rng)
...
def test_mutate_stays_in_bounds(ff):
    rng = random.Random(3)
    snap = _random_within_bounds(ff, rng)
    _mutate(snap, ff, rate=1.0, sigma_frac=2.0, rng=rng)   # extreme σ — anything still clamps
    for i, key in enumerate(snap.keys):
        b = ff.param_min_max_delta[key]
        assert b["min"] <= snap.values[i] <= b["max"]
...
def test_ga_loop_drives_synthetic_objective_to_minimum(ff):
    ...
    pop_size = 20
    population = [_random_within_bounds(ff, rng) for _ in range(pop_size)]
    ...
    for _ in range(40):
        ...
        while len(new_pop) < pop_size:
            p1 = _tournament_pick(population, costs, k=3, rng=rng)
            p2 = _tournament_pick(population, costs, k=3, rng=rng)
            c1, c2 = _crossover(p1, p2, rate=0.7, rng=rng)
            _mutate(c1, ff, rate=0.3, sigma_frac=0.1, rng=rng)
            _mutate(c2, ff, rate=0.3, sigma_frac=0.1, rng=rng)
            for c in (c1, c2):
