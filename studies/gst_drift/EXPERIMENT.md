...
| 2026-05-23 | **⚠️ IMPORTANT — salvaged `GST_rocksalt` vc-relax cache entry is at *pre-bump* settings; full re-run required for v1 training to be coherent**. Diagnostic surfaced while sanity-checking the strain-scan ΔE table printed after the populator finished its long run. Pattern: the hydrostatic ladder reads `+138, +56, +29, −12, −49, −63, −73` kcal/mol from ε=−0.06 to ε=+0.06 — *monotonically* descending into tension instead of forming a bowl with min at ε=0. Same pattern in uni_z and bi_xy: every direction wants to expand. Verified the cause by reading the cache directly: `GST_rocksalt` vc-relax cached energy is `−691670.87` kcal/mol but a fresh single_point at the *exact same atoms+box* gives `−691716.06` kcal/mol — 45 kcal/mol gap on identical geometry. Two SCFs can't disagree by 45 kcal/mol at fixed inputs unless they're at different settings, so the cached vc-relax is at *old* settings (ecutwfc=40, Γ-only, conv_thr=1e-7) — exactly what the 2026-05-10 rescue script wrote in from `/tmp/qe_vcrelax_failed_qe_vcrelax_ncyvq0bo/`. The cache key was re-computed after the 2026-05-09 production-stack bump and happens to match the current key, so `cache.has()` returns True — the mismatch is silent. The +12 kcal/mol residual at ε=0 is the new-settings BFGS relaxing the atoms further inside the same box (vc-relax atoms are at a Γ-only minimum, atoms-only BFGS at 2×2×2 finds a lower minimum). The other 19 strain-point relaxes are all at the production stack and are consistent with each other — they just disagree with the reference. **Fix is the slow one**: re-run `GST_rocksalt` vc-relax at the production stack, then re-do `make-scan` + `populate_qm`. New reference geometry → new content hash → all 19 strain points re-key and re-run (~10+ days on 8 ranks; ~1.5 days on a 64-core node). **Action taken in this session** (no compute spent): (1) `scripts/invalidate_qm_cache.py` — generic invalidator that deletes both the vc-relax cache entry *and* the orphan single_point entry stored at its post-relax geometry; chases the latter by loading the cached relax result before deletion to get the geometry it was keyed on. (2) `pyfield/qm/qe_backend.py:_detect_cpus` + `{nproc}` placeholder substitution in `ESPRESSO_COMMAND` — so the same exported command works on the WSL laptop (16 logical cores) and the 64-core rerun box without re-editing; respects `sched_getaffinity` so it does the right thing inside a SLURM allocation too. (3) README.md and DEV.md updated to document the `{nproc}` token; this entry written. **What the user should do on the 64-core machine**: `git pull`, set `export ESPRESSO_COMMAND='/usr/bin/mpirun.openmpi -np {nproc} pw.x'`, run `python scripts/invalidate_qm_cache.py --cfg studies/gst_drift/gst_drift.yaml --structure GST_rocksalt --dry-run` to confirm 2 entries will go, then re-run without `--dry-run`, then re-run the qm-prep cell. Forward note: still worth re-seeding `GST_rocksalt.box` from `[6.02]·3` to `~[7.65]·3` in the source YAML for v2 so future re-relaxes start near the equilibrium (shorter wall-clock, no shear projection); the current seed sits at 12 Å³/atom (double-compressed) and PBE has to expand 28 % linearly to relieve it. |
